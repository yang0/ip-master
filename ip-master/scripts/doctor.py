#!/usr/bin/env python3
"""Read-only diagnostic for IP Master characters and optional Skills.
...
def _candidate_paths(
    dependency: dict[str, Any], *, skill_dir: Path = SKILL_DIR
) -> list[Path]:
...
def _load_registry(*, skill_dir: Path = SKILL_DIR) -> dict[str, Any]:
    return load_dependency_registry(skill_dir / "references" / "skill-registry.json")
...
def diagnose(
    *,
    ...
    registry = _load_registry(skill_dir=skill_dir) if registry is None else registry
    characters = inspect_character_registry(skill_dir=skill_dir)
    dependencies = [
        inspect_dependency(
            dependency,
    ...
    return {
        "registry": str(skill_dir / "references" / "skill-registry.json"),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Read-only IP Master dependency doctor")
