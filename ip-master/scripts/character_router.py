...
def _registry_path(skill_dir: Path = SKILL_DIR) -> Path:
    """Return the character registry for an installed Skill root."""

    return skill_dir.expanduser().resolve(strict=False) / "references" / "character-registry.json"


def _read_registry(registry_path: Path | None = None) -> dict[str, Any]:
    registry_path = REGISTRY_PATH if registry_path is None else registry_path
...
def _resolve_skill_path(relative_path: str, skill_dir: Path = SKILL_DIR) -> Path:
...
def inspect_registry(*, skill_dir: Path = SKILL_DIR) -> dict[str, Any]:
    """Return a JSON-safe validation report without changing the filesystem."""

    report: dict[str, Any] = {
        "registry": str(_registry_path(skill_dir)),
        "version": None,
    ...
    try:
        registry = _read_registry(_registry_path(skill_dir))
    ...
    for index, item in enumerate(characters):
        ...
        asset_path = _resolve_skill_path(asset, skill_dir) if isinstance(asset, str) else None
        identity_path = (
            _resolve_skill_path(identity_reference, skill_dir)
...
def _load_validated_registry(*, skill_dir: Path = SKILL_DIR) -> dict[str, Any]:
    report = inspect_registry(skill_dir=skill_dir)
    if not report["valid"]:
        supported = [
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Resolve IP Master character names")
