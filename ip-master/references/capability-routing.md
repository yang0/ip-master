...
不会在首轮海报或高密度图中自动生效。只有用户在返图后明确选择编号（如
`23`、`023` 或 `layout-023`）重排时，才将对应的构图方法论传给原先选定的
海报 Skill。缩略图只供用户浏览，不能作为生图图片引用；方法论会按新主题
重新落位，不得继承样图坐标或视觉元素。最终实际图片为竖版时，交付可附上
本地画廊链接；横图和只输出提示词的 Skill 不附。
